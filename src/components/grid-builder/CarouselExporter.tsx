...
export default function CarouselExporter({ layout, cells, gridRef, textLayers = [] }: Props) {
  ...
  // Strict: count only valid mapped cells
  const filledCells = layout.cells.map((_, i) => cells[i]).filter((c) => c?.imageUrl);
  ...
  const exportSlides = async () => {
    ...
    try {
      for (let i = 0; i < layout.cells.length; i++) {
        ...
        ctx.drawImage(img, (1080 - w) / 2 + offsetX * (1080 / 440), (1080 - h) / 2 + offsetY * (1080 / 440), w, h);

...
export function WebsiteAbout({ template, branding, id }: WebsiteAboutProps) {
  ...
  return (
    <section id={id} className="py-24 sm:py-32 px-6" style={{ backgroundColor: tmpl.bg }}>
      <div className={`max-w-5xl mx-auto ${hasCover ? 'grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 items-center' : ''}`}>
        {/* Image side */}
        {hasCover && (
          <div className="relative overflow-hidden rounded-sm" style={{ aspectRatio: '4/5' }}>
            ...
            <div
              ...
              style={{ boxShadow: `inset 0 0 60px ${tmpl.bg}33` }}
            />
          </div>
        )}

        {/* Text side */}
        <div className={hasCover ? 'space-y-6' : 'max-w-3xl mx-auto text-center space-y-8'}>
          ...
          <div className={`w-10 h-[1px] ${hasCover ? '' : 'mx-auto'}`} style={{ backgroundColor: accent, opacity: 0.4 }} />
          <p className="text-sm sm:text-base leading-[1.9]" style={{ color: tmpl.textSecondary, fontFamily: tmpl.uiFontFamily }}>
            {branding.bio}
          </p>
          <div className={`flex items-center gap-6 pt-4 ${hasCover ? '' : 'justify-center'}`}>
            {branding.instagram && (

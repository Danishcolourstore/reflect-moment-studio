...
const Cheetah = () => {
  ...
  const statCards = [
    ...
    { label: 'Downloads', value: 67, icon: Download },
    { label: 'Views', value: 218, icon: Eye },
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h1 className="font-serif text-xl font-semibold text-foreground tracking-tight">Cheetah</h1>
      ...
      {/* Live indicator */}
      <div className="h-[2px] rounded-full bg-gradient-to-r from-accent via-primary to-accent mb-4 opacity-60" />
          ...
          <div className="space-y-1.5">
            {LIVE_VIEWERS.map((lv) => (
              <div key={lv.event} className="flex items-center justify-between">
                <p className="text-[13px] text-foreground truncate mr-2">
                  <span className="font-semibold">{lv.viewers}</span> in <span className="font-medium">{lv.event}</span>
                </p>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        {statCards.map((s) => (
          <div key={s.label} className="bg-card border border-border rounded-xl p-3 flex items-center gap-2.5">
            <div className="h-8 w-8 rounded-lg bg-secondary flex items-center justify-center text-foreground shrink-0">
              <s.icon className="h-4 w-4" />
            </div>
            <div className="min-w-0">
              <p className="text-lg font-semibold text-foreground font-serif leading-tight">{s.value}</p>
      ...
      <div className="flex gap-1 bg-secondary/50 rounded-lg p-1 mb-4 overflow-x-auto">
        {([
          { key: 'feed', label: 'Feed', icon: Activity },
          { key: 'events', label: 'Events', icon: Image },
          { key: 'timeline', label: 'Timeline', icon: Clock },
        ] as const).map((tab) => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key)}
            className={`flex-1 min-w-0 flex items-center justify-center gap-1.5 py-2.5 rounded-md text-[11px] font-medium tracking-wide transition-all min-h-[44px] ${
              activeTab === tab.key
        ...
        <div className="space-y-2">
          {DUMMY_ACTIVITIES.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="flex items-start gap-3 bg-card border border-border rounded-xl px-3 py-3 active:bg-secondary/30 transition-colors"
              >
      ...
      {activeTab === 'events' && (
        <div className="space-y-3">
          {EVENT_MONITORS.map((ev) => (
            <div key={ev.name} className="bg-card border border-border rounded-xl p-3">
              <div className="flex items-center justify-between mb-2.5">
                <h3 className="text-[13px] font-medium text-foreground truncate mr-2">{ev.name}</h3>
                {ev.activeViewers > 0 && (
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  { val: ev.activeViewers, label: 'Viewers' },
                  { val: ev.recentDownloads, label: 'Downloads' },
                  { val: ev.totalVisitors, label: 'Visitors' },
                ].map((stat) => (
                  <div key={stat.label} className="text-center">
                    <p className="text-base font-semibold text-foreground font-serif">{stat.val}</p>

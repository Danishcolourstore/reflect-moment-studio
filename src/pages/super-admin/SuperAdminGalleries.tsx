...
import {
  Image, Settings, Layout, Shield, Palette, BarChart3, Heart,
  Save, Plus, Trash2, Eye, Download, Lock, Grid3X3, Rows3,
  Columns3, Maximize, BookOpen, ChevronRight, ToggleLeft,
} from 'lucide-react';
...
function useGallerySettings() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      (data ?? []).forEach((r: any) => { if (r.key.startsWith(SETTINGS_PREFIX)) map[r.key] = r.value; });
      return map;
    },
  });
}
    ...
    <div className="p-4 md:p-8 space-y-6 max-w-6xl">
      ...
      <Tabs defaultValue="settings" className="space-y-4">
        <TabsList className="flex flex-wrap h-auto gap-1 bg-muted/50">
          ...
          <TabsTrigger value="layouts" className="gap-1.5 text-xs"><Layout className="h-3.5 w-3.5" />Layouts</TabsTrigger>
          <TabsTrigger value="templates" className="gap-1.5 text-xs"><BookOpen className="h-3.5 w-3.5" />Templates</TabsTrigger>
          <TabsTrigger value="selection" className="gap-1.5 text-xs"><Heart className="h-3.5 w-3.5" />Selection</TabsTrigger>
        ...
        <TabsContent value="layouts"><LayoutsTab /></TabsContent>
...
function GlobalSettingsTab() {
  ...
  const save = async () => {
    ...
    for (const [k, v] of pairs) await upsert.mutateAsync({ key: `${SETTINGS_PREFIX}${k}`, value: v });
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Field label="Auto-Expiry (days, 0 = never)">
            <Input type="number" min={0} value={expiryDays} onChange={e => setExpiryDays(e.target.value)} className="bg-background" />
          </Field>
        </div>
...
function LayoutsTab() {
  ...
  const toggle = async (id: string) => {
    const next = enabledLayouts.includes(id) ? enabledLayouts.filter(l => l !== id) : [...enabledLayouts, id];
    await upsert.mutateAsync({ key: `${SETTINGS_PREFIX}enabled_layouts`, value: JSON.stringify(next) });
    toast.success(`Layout ${next.includes(id) ? 'enabled' : 'disabled'}`);
  };
    ...
    <div className="space-y-4">
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {LAYOUT_PRESETS.map(l => {
              const on = enabledLayouts.length === 0 || enabledLayouts.includes(l.id);
              return (
                <button
                  key={l.id}
                  onClick={() => toggle(l.id)}
                ...
                >
                  ...
                  <Badge className={cn('absolute top-2 right-2 text-[9px]', on ? 'bg-primary/15 text-primary' : 'bg-muted text-muted-foreground')}>
                    {on ? 'Active' : 'Off'}
                  </Badge>
                </button>
              );
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <SettingSlider settingsKey="layout_border_radius" label="Thumbnail Radius (px)" min={0} max={24} defaultVal={4} />
        </CardContent>
      </Card>
    </div>
  );
...
const GALLERY_TEMPLATE_PRESETS = [
  { id: 'wedding', name: 'Wedding Gallery', style: 'timeless-wedding', layout: 'masonry', download: 'web', selection: true },
  { id: 'pre-wedding', name: 'Pre Wedding Gallery', style: 'vogue-editorial', layout: 'editorial', download: 'web', selection: true },
  { id: 'birthday', name: 'Birthday Event', style: 'vogue-editorial', layout: 'grid', download: 'full', selection: false },
  { id: 'portfolio', name: 'Portfolio Gallery', style: 'andhakar', layout: 'cinematic', download: 'disabled', selection: false },
  { id: 'proofing', name: 'Client Proofing', style: 'vogue-editorial', layout: 'justified', download: 'web', selection: true },
];
...
function TemplatesTab() {
  ...
  const enabledTemplates: string[] = (() => {
    try { return JSON.parse(settings?.[`${SETTINGS_PREFIX}enabled_templates`] ?? '[]'); } catch { return GALLERY_TEMPLATE_PRESETS.map(t => t.id); }
  })();

  const toggle = async (id: string) => {
    const next = enabledTemplates.includes(id) ? enabledTemplates.filter(t => t !== id) : [...enabledTemplates, id];
      ...
      <CardContent>
        <div className="space-y-2">
          {GALLERY_TEMPLATE_PRESETS.map(t => {
            const on = enabledTemplates.length === 0 || enabledTemplates.includes(t.id);
            return (
              <div key={t.id} className={cn('flex items-center justify-between p-3 rounded-lg border transition-all', on ? 'border-primary/30 bg-primary/5' : 'border-border bg-card opacity-60')}>
                <div className="flex items-center gap-3">
      ...
      <CardContent className="space-y-3">
        {features.map(f => (
          <div key={f.key} className="flex items-center justify-between p-3 rounded-lg border border-border bg-card">
            ...
            <Switch checked={isOn(f.key)} onCheckedChange={() => toggle(f.key)} />
          </div>
        ))}
      </CardContent>
    </Card>
...
function BrandingTab() {
  ...
  const isOn = (key: string) => (settings?.[`${SETTINGS_PREFIX}${key}`] ?? 'true') === 'true';
  const toggle = async (key: string) => {
    await upsert.mutateAsync({ key: `${SETTINGS_PREFIX}${key}`, value: String(!isOn(key)) });
  ...
  return (
    <div className="space-y-4">
        ...
        <CardContent className="space-y-3">
          {options.map(o => (
            <div key={o.key} className="flex items-center justify-between p-3 rounded-lg border border-border bg-card">
              ...
              <Switch checked={isOn(o.key)} onCheckedChange={() => toggle(o.key)} />
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <SettingSelect settingsKey="brand_header_layout" label="Header Style" options={[
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        <SettingSelect settingsKey="sec_default_download_mode" label="Default Download Mode" options={[
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="text-sm text-muted-foreground">
          Analytics are aggregated from all photographer galleries in real-time via the event analytics system.
          Detailed per-gallery breakdowns are available in each photographer's dashboard.
        </CardContent>
      </Card>
    </div>
  );
...
function SettingSelect({ settingsKey, label, options, defaultVal }: { settingsKey: string; label: string; options: { value: string; label: string }[]; defaultVal: string }) {
  const { data: settings } = useGallerySettings();
  const upsert = useUpsertSetting();
  ...
  return (
    <Field label={label}>
      <Select value={val} onValueChange={(v) => { upsert.mutate({ key: `${SETTINGS_PREFIX}${settingsKey}`, value: v }); }}>
        <SelectTrigger><SelectValue /></SelectTrigger>
        <SelectContent>

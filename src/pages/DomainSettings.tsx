...
import { Copy, Plus, Globe, Check, Trash2, Loader2, Lightbulb, Info, ExternalLink, AlertTriangle } from "lucide-react";
import {
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="relative ml-2 p-1 rounded hover:bg-[#E8E0D4]/50 transition-colors"
      title="Copy"
    >
      {copied ? <Check className="w-3.5 h-3.5 text-green-600" /> : <Copy className="w-3.5 h-3.5 text-[#1A1A1A]/50" />}
      {copied && (
        <span className="absolute -top-7 left-1/2 -translate-x-1/2 text-xs bg-[#1A1A1A] text-white px-2 py-0.5 rounded whitespace-nowrap">
          Copied!
        </span>
      )}
    </button>
...
export default function DomainSettings() {
  ...
  const validateDomain = (v: string) => {
    const cleaned = v.replace(/^https?:\/\//, "").trim();
    return cleaned.includes(".") && !/\s/.test(cleaned) ? cleaned : null;
  };

  const handleAddCustom = async () => {
    const domain = validateDomain(customInput);
    if (!domain) { toast({ title: "Invalid domain", description: "Enter a valid domain like www.example.com", variant: "destructive" }); return; }
    setSaving(true);
    const { error } = await (supabase.from("domains").insert({ user_id: user!.id, subdomain: subdomain + "-custom", custom_domain: domain, verification_status: "pending" }) as any);
    setSaving(false);
    if (error) {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        toast({ title: "Verification failed", description: data?.message || "DNS records not detected yet.", variant: "destructive" });
      }
    } catch (e: any) {
        ...
        <CardContent>
          <div className="flex items-center gap-2">
            <a
              ...
              className="text-[#C9A96E] hover:underline font-medium flex items-center gap-1"
        ...
        <CardContent className="space-y-4">
          {!customRow && !showAddForm && (
            <Button
              ...
              className="border-[#E8E0D4] text-[#1A1A1A] hover:bg-[#FDFBF7]"
            >
          ...
          {showAddForm && !customRow && (
            <div className="flex gap-2">
              <Input
                ...
                className="border-[#E8E0D4] bg-[#FDFBF7]"
                style={{ fontFamily: "Inter, sans-serif" }}
              />
              <Button onClick={handleAddCustom} disabled={saving} className="bg-[#C9A96E] hover:bg-[#b8985d] text-white">
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : "Connect Domain"}
              </Button>
              <Button variant="ghost" onClick={() => { setShowAddForm(false); setCustomInput(""); }}>Cancel</Button>
            </div>
          )}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-[#1A1A1A]" style={{ fontFamily: "Inter, sans-serif" }}>
                ...
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" onClick={handleVerify} disabled={verifying} className="border-[#E8E0D4]">
        ...
        <Card className="border-[#E8E0D4] bg-white shadow-sm">
          ...
          <CardContent className="space-y-4">
            <Tabs defaultValue="cname">
              <TabsList className="bg-[#FDFBF7] border border-[#E8E0D4]">
                <TabsTrigger value="cname" className="data-[state=active]:bg-white data-[state=active]:text-[#C9A96E]">
                  CNAME Record (Recommended)
                </TabsTrigger>
                <TabsTrigger value="a" className="data-[state=active]:bg-white data-[state=active]:text-[#C9A96E]">
                  A Record (Root Domain)
                </TabsTrigger>
              </TabsList>

              <TabsContent value="cname" className="mt-4">
                <div className="rounded-lg border border-[#E8E0D4] overflow-hidden" style={{ fontFamily: "Inter, sans-serif" }}>
                  <table className="w-full text-sm">
                    <thead className="bg-[#FDFBF7]">
                      <tr>
                        <th className="text-left p-3 font-medium text-[#1A1A1A]/60">Field</th>
                        <th className="text-left p-3 font-medium text-[#1A1A1A]/60">Value</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr className="border-t border-[#E8E0D4]">
                        <td className="p-3 text-[#1A1A1A]/70">Type</td>
                        <td className="p-3 font-mono text-sm">CNAME</td>
                      </tr>
                      <tr className="border-t border-[#E8E0D4]">
                        <td className="p-3 text-[#1A1A1A]/70">Host</td>
                        <td className="p-3 flex items-center font-mono text-sm">www <CopyButton text="www" /></td>
                      </tr>
                      <tr className="border-t border-[#E8E0D4]">
                        <td className="p-3 text-[#1A1A1A]/70">Value</td>
                        <td className="p-3 flex items-center font-mono text-sm">mirroraigallery.com <CopyButton text="mirroraigallery.com" /></td>
                      </tr>
                      <tr className="border-t border-[#E8E0D4]">
                        <td className="p-3 text-[#1A1A1A]/70">TTL</td>
          ...
          <CardContent className="space-y-3">
            {[subdomainRow, customRow].map(d => (
              ...
              >
                ...
                {d.is_primary && <Check className="w-4 h-4 text-[#C9A96E]" />}
              </button>
            ))}
          </CardContent>
        </Card>

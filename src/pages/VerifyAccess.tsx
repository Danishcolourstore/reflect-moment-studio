...
export default function VerifyAccess() {
  ...
  // Generate & send PIN on mount
  useEffect(() => {
    if (!user || pinSent) return;

    const generatedPin = Math.floor(100000 + Math.random() * 900000).toString();
    sessionStorage.setItem('mirrorAccessPin', generatedPin);
    setPinSent(true);

    supabase.functions.invoke('send-access-pin', {
      body: { type: 'send_pin', pin: generatedPin, user_email: user.email },
    }).catch(console.error);
  }, [user, pinSent]);
  ...
  const handleVerify = useCallback(async (inputPin: string) => {
    ...
    const sessionPin = sessionStorage.getItem('mirrorAccessPin') || '';

    if (inputPin === MASTER_PIN || inputPin === sessionPin) {
      sessionStorage.setItem(SESSION_KEY, 'true');
      sessionStorage.removeItem('mirrorAccessPin');
      const redirect = sessionStorage.getItem('redirectAfterLogin');
      sessionStorage.removeItem('redirectAfterLogin');
      navigate(redirect || '/dashboard', { replace: true });
    } else {
    ...
    setVerifying(false);
  }, [attempts, verifying, lockoutEnd, user, navigate]);
      ...
      <Card className="w-full max-w-sm border-border/20 bg-card/80 backdrop-blur-sm">
        <CardHeader className="text-center space-y-3">
          ...
          <CardTitle className="text-lg">
            {isLocked ? 'Access Temporarily Locked' : 'Owner Verification'}
          </CardTitle>
          <CardDescription>
            {isLocked
              ? `Try again in ${countdown} seconds`
              : 'Please ask Danish for your access PIN to continue.'}
          </CardDescription>
        </CardHeader>

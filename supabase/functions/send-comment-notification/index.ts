    ...
    if (RESEND_API_KEY && profile?.email) {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html: `<div style="font-family:serif;max-width:600px;margin:0 auto;padding:40px 20px"><h1 style="font-size:24px">${guest_name || 'A guest'} commented on your photo</h1><blockquote style="border-left:3px solid #b0875a;padding-left:16px;color:#666;font-style:italic">${content || ''}</blockquote><p style="color:#666">Photo in <strong>${event.name}</strong></p><p style="margin-top:40px;color:#999;font-size:12px">— MirrorAI</p></div>`,
        }),
      });
    }
